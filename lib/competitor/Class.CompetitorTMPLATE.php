...
class Competitor5 extends CompetitorBase {
    ...
    {
        preg_match_all("/class=\"cpdesc less\">([^<]+?)<\/span>/",$couponItem,$matchCouponTitle);
        return empty($matchCouponTitle[0])?'':$matchCouponTitle[1][0];
    }
    //couponDesc
    public function couponDesc($couponItem)
    {
        preg_match_all("/class=\"coupon_title\" id=\"coupontitle_\d+\" infos=\"\d+_n\">([^<]+?)<\/div>/",$couponItem,$matchCouponDesc);
